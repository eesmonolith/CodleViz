...
def session_timeline(df: pd.DataFrame, title: str = "",
    ...
    fig.update_layout(
        ...
        xaxis=dict(title=None, dtick=1, gridcolor="#F1F5F9", tickfont=dict(size=11)),
...
def student_heatmap(df: pd.DataFrame, title: str = "") -> go.Figure:
    ...
    fig.update_layout(
        ...
        yaxis=dict(title=None, autorange="reversed", tickfont=dict(size=10)),
...
def activity_type_stacked(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        ...
        xaxis=dict(title=None, dtick=1, tickfont=dict(size=11), showgrid=False),
        yaxis=dict(title=None, range=[0, 100], ticksuffix="%", showgrid=True, gridcolor="#F1F5F9"),
        height=380,
        legend=dict(
...
def trajectory_sparklines(session_df: pd.DataFrame, summary_df: pd.DataFrame,
    ...
    fig.update_layout(
        ...
        title=dict(text=title, x=0.5, font=dict(size=14)),
        barmode="group",
        xaxis=dict(title=None, range=[0, 105], gridcolor="#F1F5F9", ticksuffix="%"),
        yaxis=dict(title=None, autorange="reversed", tickfont=dict(size=9)),
