...
if view_level == "Overview":
    ...
    with tab1:
        ...
        fig = school_comparison_bar(_translate_df(filtered_summary), lang="en")
    ...
    with tab2:
        ...
        fig = competency_comparison_grouped(_translate_df(comp_filtered), lang="en")
    ...
    with tab3:
        ...
        for curr in selected_curricula:
            ...
            if len(curr_data) > 0:
                fig = activity_type_stacked(_translate_df(curr_data), lang="en")
    ...
    with tab4:
        ...
        fig = cliff_heatmap(_translate_df(filtered_session), _translate_df(summary), threshold=cliff_threshold, lang="en")
    ...
    with tab5:
        ...
        fig = trajectory_alignment(_translate_df(filtered_session), _translate_df(summary), lang="en")
        ...
        fig2 = trajectory_sparklines(_translate_df(filtered_session), _translate_df(summary), lang="en")
...
# ══════════════════════════════════════════════════════
    ...
    for i, (_, row) in enumerate(school_classrooms.iterrows()):
        ...
        with cols[i % len(cols)]:
            fig = competency_radar(comp_data, title=_name_en(row["classroom_name"]), lang="en")
            show_chart(fig)
    ...
    for _, row in school_classrooms.iterrows():
        ...
        if len(sess_data) > 0:
            fig = session_timeline(sess_data, title=_name_en(row["classroom_name"]), lang="en")
            show_chart(fig)
...
# ══════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        fig = competency_radar(comp_data, title="5 Competency Scores", lang="en")
        show_chart(fig)
    ...
    with tab3:
        ...
        if len(heat_data) > 0:
            fig = student_heatmap(heat_data, title="Student Session Progress", lang="en")
            show_chart(fig)
    ...
    with tab4:
        ...
        heat_data = get_heatmap_for_classroom(selected_classroom)
        if len(heat_data) > 0:
            fig = dependency_scatter(heat_data, title=f"Learning Type Classification — {_name_en(selected_classroom)}", lang="en")
            show_chart(fig)
